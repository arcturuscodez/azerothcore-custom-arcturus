...
-- ============================================================================
-- ALE SESSION CACHE (Eluna GetData / SetData shim)
-- ============================================================================
-- mod-ale does not bind Object:GetData / SetData. Paragon used those to keep the
-- in-memory Paragon instance on the player. Without a shim, every login/XP/kill/
-- logout errors on the world thread (amplified under playerbots).
-- Prefer native methods when present; otherwise key by character GUID low.

local ParagonSession = {} -- [guidLow] = paragon

local function ParagonGet(player)
    if not player then
        return nil
    end
    ...
    return ParagonSession[player:GetGUIDLow()]
end

local function ParagonSet(player, paragon)
    if not player then
        return
    end
    ...
    ParagonSession[player:GetGUIDLow()] = paragon
end

local function ParagonClear(player)
    if not player then
        return
    end
    if player.SetData then
        -- Eluna stores nil as clearing the key in practice for this use.
        player:SetData("Paragon", nil)
        return
    end
    ParagonSession[player:GetGUIDLow()] = nil
end

local function ParagonClearGuid(guid_low)
    if guid_low then
        ParagonSession[guid_low] = nil
    end
end

-- Exported for sibling modules (e.g. paragon_target_level).
Hook.ParagonGet = ParagonGet
Hook.ParagonSet = ParagonSet
Hook.ParagonClear = ParagonClear
...
function OnParagonClientLoadRequest(player, _)
    ...
    local paragon = ParagonGet(player)
    if not paragon then
...
function Hook.OnPlayerLogout(event, player)
    ...
    -- Drop session cache so GUID reuse cannot leak state (bots / re-login).
    ParagonClear(player)
end
