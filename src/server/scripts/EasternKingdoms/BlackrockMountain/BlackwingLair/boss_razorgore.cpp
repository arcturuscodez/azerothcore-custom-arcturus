 ...
 */
 ...
 struct boss_razorgore : public BossAI
 {
    ...
    {
        // Always treat Razorgore's death as a successful encounter,
        // regardless of whether the eggs were destroyed.
    ...
    void DoChangePhase()
    {
     ...
     {
         // If Razorgore would die before the eggs are destroyed,
         // skip the original raid-wide instant-kill mechanic.
         //
         // Instead, force him into phase 2 and leave him at 1 HP.
         // This allows the player/bots to kill him normally afterward,
         // which causes the normal _JustDied() loot handling to occur.
     ...
     bool GossipHello(Player* player, bool /*reportUse*/) override
     {
         if (InstanceScript* instance = me->GetInstanceScript())
             if (instance->GetData(DATA_EGG_EVENT) != DONE &&
                 !player->HasAura(SPELL_MIND_EXHAUSTION) &&
                 !player->GetPet())
                 if (Creature* razor = ObjectAccessor::GetCreature(
                         *me,
                         instance->GetGuidData(DATA_RAZORGORE_THE_UNTAMED)))
                 {
